...
class ClusterEnv(gym.Env[dict, np.ndarray]):
    ...
    tick_action_option: tp.Annotated[
        PositiveInt, Doc("Either 0 for no time skip and 1 otherwise")
    ] = field(init=False, default=2)
    current_clock_tick: tp.Annotated[
        NonNegativeInt, Doc("Total ticks from start of the cluster")
    ] = field(init=False, default=0)
    ...
    def __post_init__(self):
        ...
        logger.info(
            f"Cluster env created with {self.n_machines=} {self.n_jobs=} {self.n_resources=} {self.n_ticks=}"
        )
        self.action_space = gym.spaces.MultiDiscrete(
            np.array([self.tick_action_option, self.n_machines, self.n_jobs])
        )
        self.observation_space = self.generate_cluster_observation_shape(
            CLUSTER_CELL_HIGH
        )
    ...
    def generate_cluster_observation_shape(self, high: float) -> gym.spaces.Dict:
        jobs_shape = gym.spaces.Box(
            low=0,
            high=high,
            shape=(self.n_jobs, self.n_resources, self.n_ticks),
            dtype=np.uint8,
        )
        job_status_shape = gym.spaces.Box(
            low=0, high=high, shape=(self.n_jobs,), dtype=np.uint8
        )
        machines_shape = gym.spaces.Box(
            low=0,
            high=high,
    ...
    def validate_job_status(self, j: JobIndex) -> IsSucceed:
        ...
        if job_status != JobStatus.Pending:
            logger.debug(
                f"Failed Scheduling job index {j} with status {job_status} should be {JobStatus.Pending}"
            )
            return False
        ...
        if machine_end_tick <= self.n_ticks:
            ...
            if np.all((free_space - job_utlization) >= 0):
                self.machines.free_space[
                    m, :, current_tick : current_tick + job_length
                ] -= job_utlization
                return True
        return False
    ...
    def step(
    ...
    ) -> tuple[dict, tp.SupportsFloat, bool, bool, dict[str, tp.Any]]:
        """tp.Tuple[TickAction, MacineIndex, JobIndex]"""
        should_tick, machine_idx, job_idx = action
        # Handle clock ticking
        if should_tick:
            logger.debug(
                f"Foward time, {self.current_clock_tick} -> {self.current_clock_tick + 1}"
            )
        ...
        if not self.schedule(machine_idx, job_idx):
            logger.debug(
                f"Cannot allocate job {job_idx} to machine {machine_idx} not enough free space"
            )
            return (
                self.observation,
                0.0,
        ...
        logger.debug(
            f"Schedule machine.{machine_idx}, job.{job_idx} on {self.current_clock_tick} tick"
        )
