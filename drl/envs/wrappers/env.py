...
class ScheduleFromSelectedTimeWrapper(Wrapper):
    ...
    def schedule(
        ...
        job_utlization_block = self.jobs.utlization[j, :, job_arrival_time:job_arrival_time + job_length]
        # Extract machine availability for the duration from current tick to max time allowed
        machine_free_space = self.machines.free_space[m, :,
                             self.current_clock_tick:(self.current_clock_tick + job_length)]
        # Slide over the free space array to check blocks of size job_length for resource sufficiency
        free_space_slices = np.lib.stride_tricks.sliding_window_view(machine_free_space,
                                                                     window_shape=job_utlization_block.shape[1:],
                                                                     axis=1)
        # Check if there's any time block where all resources meet the job's requirements
        sufficient_space = np.all(free_space_slices >= job_utlization_block, axis=(1, 2))
        if np.any(sufficient_space):
            ...
            self.machines.free_space[m, :, start_time:start_time + job_length] -= job_utlization_block
            return True
        return False
...
class QueueWrapper(Wrapper):
    ...
    def __init__(self, env: ClusterEnv, *, queue_size: PositiveInt):
        if queue_size <= 0:
            raise ValueError(f"{type(self).__name__} queue size should be postive bigger than zero. not {queue_size}")
        super().__init__(env)
    ...
    def modify_observation(self, observation: ClusterBaseOvservation) -> ClusterBaseOvservation:
        ...
        observation["status"] = observation["status"][self.job_indexer][:self.queue_size]
        return observation

    def step(self, action: np.ndarray) -> tuple[ClusterObservation, tp.SupportsFloat, bool, bool, dict[str, tp.Any]]:
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, tp.Any] | None = None) -> tuple[
        ClusterObservation, dict[str, tp.Any]]:
