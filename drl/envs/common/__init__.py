...
class Machines:
    ...
    n_ticks: tp.Annotated[PositiveInt, Doc("Maximum number of clock tick allowed in the system. In other word the maximum job size")]
    free_space: tp.Annotated[np.ndarray, Doc("Unutlize computing power, size of [machines, resource, ticks]")] = field(init=False)

    def __post_init__(self):
        self.free_space = np.full(shape=(self.n_machines, self.n_resources, self.n_ticks), fill_value=CLUSTER_CELL_HIGH, dtype=np.uint8)
...
class Jobs:
    ...
    n_resources: tp.Annotated[PositiveInt, Doc("Number of resource in each machine (cpu, ram, disk, netowrk, etc...)")]
    n_ticks: tp.Annotated[PositiveInt, Doc("Maximum number of clock tick allowed in the system. In other word the maximum job size")]
    utlization: tp.Annotated[np.ndarray, Doc("Space utlization of job, size of [jobs, resource, ticks]")] = field(init=False)
    arrival_time: tp.Annotated[np.ndarray, Doc("One dim arrray of job arrival time")] = field(init=False)
    ...
    runnning_time: tp.Annotated[np.ndarray, Doc("Running time for each job (1d array)")] = field(init=False)
    ...
    def __post_init__(self):
        ...
        jobs_end_time = self.n_ticks - get_first_none_zero_value(np.flip(self.utlization, axis=-1))
        self.length = jobs_end_time - self.arrival_time
        self.status = np.full(shape=(self.n_jobs), fill_value=JobStatus.NotCreated, dtype=np.uint8)
    ...
    def generate_jobs_utilization(self) -> np.ndarray:
        ...
        jobs_array = np.zeros((self.n_jobs, self.n_resources, self.n_ticks), dtype=np.float32)

        # Job arrival times based on Bernoulli process
        enter_times = np.cumsum(np.random.binomial(1, self._job_arrival_rate, size=self.n_jobs))
        ...
        dominant_demand = np.random.uniform(0.25, 0.5, size=self.n_jobs) * (255 / 0.5)  # Scale to [127.5, 255]
        secondary_demand = np.random.uniform(0.05, 0.1, size=self.n_jobs) * (255 / 0.5)  # Scale to [25.5, 51]
        ...
        mask = (time_indices >= enter_times[:, None]) & (time_indices < (enter_times + job_durations)[:, None])
